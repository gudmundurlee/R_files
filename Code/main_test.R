...
gen_demand_data <- function(demand_a, demand_p, ss, st, of, lt){
  ...
  data <- data.frame(demand_a = demand_a, demand_p = demand_p, est_stock, acc_est_stock, delivered, is_order, is_delivery, net_demand)
...
long5.1 <- ss_sl_sim_repeat(6, lambda = 10, h = 36, lt = 30, of = 60, N = 500) # 3 months
long5.2 <- ss_sl_sim_repeat(6, lambda = 10, h = 36, lt = 60, of = 30, N = 500) # 3 months

print_result(long1)
print_result(long2)
print_result(long3)
print_result(long4)
print_result_median(long4)
print_result(long5)
print_result(long6)
...
print_result_median <- function(res){
  CF <-colnames(res$SafteyStock)
  SS <- signif(unname(colMeans(res$SafteyStock)),2)
  SL <- signif(unname(colMedian(res$ServiceLevel)),2)
  return(kable(cbind(CF,SS,SL)))
}
